...
export default function Home() {
  ...
  useEffect(() => { fetchData(industryValue) }, [industryValue])
  ...
  const trendLabel = (trend: number) => {
    if (trend > 0) return <span className="text-xs text-red-400 ml-1">↑ {trend} from yesterday</span>
    if (trend < 0) return <span className="text-xs text-green-400 ml-1">↓ {Math.abs(trend)} from yesterday</span>
    return <span className="text-xs text-gray-500 ml-1">same as yesterday</span>
  }
    ...
    <main className="min-h-screen bg-gray-950 text-white p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-gradient-to-r from-indigo-900/40 via-gray-900 to-gray-900 border border-indigo-500/20 rounded-2xl px-8 py-6 mb-8">
          ...
          <p className="text-gray-400 text-sm max-w-2xl">Real-time incident detection powered by AI. Select your industry, explore live demo incidents, or paste your own workflow data to get root cause analysis and recommended actions in under 3 seconds.</p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          <div className="bg-gray-900 rounded-xl p-4 border border-gray-800">
            ...
            <div className="flex items-baseline gap-1">
              <p className="text-2xl font-bold">{stats?.total ?? incidents.length}</p>
              {stats && trendLabel(stats.trend)}
            </div>
          </div>
          <div className="bg-gray-900 rounded-xl p-4 border border-gray-800">
            <p className="text-gray-400 text-sm mb-1">High severity</p>
            <div className="flex items-baseline gap-1">
              <p className="text-2xl font-bold text-red-400">{stats?.high_severity ?? incidents.filter(i => i.severity === 'high').length}</p>
            </div>
          </div>
          <div className="bg-gray-900 rounded-xl p-4 border border-gray-800">
            <p className="text-gray-400 text-sm mb-1">Open</p>
            <div className="flex items-baseline gap-1">
              <p className="text-2xl font-bold text-yellow-400">{stats?.open ?? incidents.filter(i => i.status === 'open').length}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          ...
          <div>
            ...
            {loading ? (
              ...
              <div className="bg-gray-900 rounded-xl p-6 border border-gray-800 text-gray-500 text-sm">No incidents found for this industry.</div>
            ) : (
                    ...
                    <div key={incident.id} className={`bg-gray-900 rounded-xl p-6 border transition-all ${isResolved ? 'border-green-500/20 opacity-75' : 'border-gray-800'}`}>
                        ...
                        <div className="flex items-center gap-2">
                          {!isResolved && (
                            <button
                              ...
                              className="px-3 py-1.5 rounded-lg text-xs font-medium bg-green-600/20 hover:bg-green-600/40 text-green-400 border border-green-500/30 disabled:opacity-50 transition-colors"
                            >
                          ...
                          >
                            {analysis?.loading ? (
                              <><svg className="animate-spin h-3 w-3" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" /></svg>Analyzing…</>
                            ) : 'Analyze with AI'}
                          </button>
                        </div>
                      </div>
                      ...
                      {analysis?.rateLimit && (
                        <div className="mt-4 p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/20 text-yellow-300 text-sm">
                          ⏳ <strong>AI rate limit reached.</strong> The free Groq tier allows ~30 requests/minute. Please wait 60 seconds and try again.
                        </div>
                      )}
                      ...
                      {/* Analysis history */}
                      <button
                        onClick={() => fetchHistory(incident.id)}
                      ...
                      {showHistoryId === incident.id && (
                        <div className="mt-2 space-y-2">
                          {history.length === 0 ? (
                            <p className="text-xs text-gray-600">No analysis history yet.</p>
                          ) : history.map(log => (
            ...
            <div className="bg-gray-900 rounded-xl border border-gray-800 p-6">
              ...
              <div className="flex rounded-lg bg-gray-800 p-1 mb-6 w-fit">
                {(['form', 'csv'] as const).map(mode => (
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${inputMode === mode ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:text-white'}`}
                  >
              ...
              {inputMode === 'form' && (
                <div className="space-y-3">
                  ...
                  <button onClick={addRow} className="text-xs text-indigo-400 hover:text-indigo-300 transition-colors mt-1">+ Add stage</button>
                </div>
              )}
              ...
              {customError && (
                <div className="mt-4 p-3 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-sm">Error: {customError}</div>
              )}
              ...
              {customResult && (
                <div className="mt-4 space-y-4">
                  {customResult.detected_issues.length > 0 ? (
                    ...
                    <div className="p-3 rounded-lg bg-green-500/10 border border-green-500/20 text-green-400 text-sm">No threshold violations detected across all stages.</div>
                  )}
